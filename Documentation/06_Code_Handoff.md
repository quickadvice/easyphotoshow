...
- **Not in git yet.** No `.gitignore`, no remote. Recommended pre-V1.0: `git init`, add a `.gitignore` covering `bin/`, `obj/`, `*.user`, `tools/ffmpeg/*.exe`, `src/EasyPhotoShow.App/Assets/Music/*.mp3`, `Errors/`. Then host (GitHub private repo or similar) for backup.
...
| Item | Notes |
|---|---|
| Music preset MP3s | Acquire royalty-free Celebration / Peaceful / Reflective tracks. Drop into `src/EasyPhotoShow.App/Assets/Music/{name}.mp3`. The csproj already has the `<None CopyToOutputDirectory>` rule, no code changes needed. Preset durations should be > 60 sec to avoid obvious looping. |
...
| Inno Setup installer | Write the `.iss` script. Bundle: the published self-contained EasyPhotoShow + tools/ffmpeg/ + Assets/Music/ + the .ico. Create Start Menu and Desktop shortcuts. Uninstaller. |
...
| Git repository | Not yet initialized. See §9. |
